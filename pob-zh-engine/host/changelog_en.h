...
inline constexpr const char* kChangelogTextEn =
	...
	u8"　·Classic POB (PoE1): the Loadouts drop-down was in English.\n"
	u8"　·New interface look settings (theme, font, zoom, background, following the launcher) did not stick; the launcher put them back.\n"
	u8"　·New interface skill sets (PoE2): one press of + added several sets, and deleting left duplicate socket groups and an error; item and config sets fixed the same way.\n"
	u8"　·New interface tree: after F2 back to English the tree was still in Chinese.\n"
	u8"　·New interface tree (PoE2): node tooltips showed an error; the heat map and compare rings were not drawn.\n"
	u8"　·New interface tree Compare: the rings appeared only after the next recalculation; the tooltip under the cursor did not update after allocating.\n"
	u8"　·New interface: with panel opacity set, dialogs, menus and drop-down lists blended into the page behind (the spectre library, drop-down options turning all white).\n"
	u8"　·New interface Notes: Ctrl+Z did nothing; a colour button over selected text deleted the text.\n"
	u8"　·New interface Skills: deleting a socket group above the main skill moved the main skill to another group; Sort gems by DPS only worked after opening the options.\n"
	u8"　·New interface: Ctrl+S on a build never saved showed an error (it now opens Save As).\n"
	u8"　·New interface Items and Skills: the right end of the item set bar and the socket group toolbar was cut off.\n"
	u8"Added\n"
	u8"　·New interface: closing the window with unsaved changes asks Save / Don't save / Cancel.\n"
	u8"　·New interface Save As: asks before overwriting an existing file.\n"
	u8"　·New interface Calcs: pick the socket group, active skill, part, stages, minion and calculation mode on the tab itself, as in classic POB.\n"
	u8"　·New interface tree: right-click an allocated jewel socket to go to that slot on the Items tab; right-click an allocated mastery (PoE1) for its effect menu.\n"
	u8"　·New interface Items: each option in a slot's drop-down shows the comparison with what is equipped when pointed at.\n"
	u8"　·New interface sidebar (PoE2): weapon set passive points.\n"
	u8"　·New interface gem search list: a line at the top says what the number on the right is.\n"
	u8"　·Translation editor: Save all shows how many edits are pending; editing a built-in dictionary warns that updates overwrite it.\n"
	u8"Changed\n"
	u8"　·New interface Items: Craft moved to the right pane, a tab beside Add item and Database.\n"
	u8"　·New interface Skills: New socket group and Delete socket group moved to the top.\n"
	u8"　·New interface: new skill, item and config sets ask for a name first.\n"
	u8"　·New interface background: brightness, panel opacity and tree backdrop work without a picture too.\n"
	u8"\n"
